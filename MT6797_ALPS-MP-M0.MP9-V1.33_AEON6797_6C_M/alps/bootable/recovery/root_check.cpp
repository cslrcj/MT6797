...
#endif
    {"/dev/uboot", "lk.bin","lk"},
    {"/dev/bootimg", "boot.img","boot"},
    {"/dev/recovery", "recovery.img","recovery"},
    {"/dev/logo", "logo.bin","logo"},
};
...
#endif
    {"/bootloader", "lk.bin","lk1",1,1},
    {"/bootloader2", "lk.bin","lk2",1,0},
    {"/boot", "boot.img","boot",0,0},
    {"/recovery", "recovery.img","recovery",1,0},
    {"/logo", "logo.bin","logo",1,0},
    {"/tee1", "trustzone.bin","tee1",1,1},
    {"/tee2", "trustzone.bin","tee2",1,0},
};
...
static int file_crc_check( const char* path, unsigned int* nCS, unsigned char *nMd5)
{
    ...
    *nCS = 0;
    ...
    if(lstat(path,&st)<0)
    {
        LOGE("\n %s does not exist,lsta fail \n", path);
        LOGE("\nlstat fail reason is %s \n",strerror(errno));
        return 1;
    }
    ...
    if( fp == NULL )
    {
        LOGE("\nfile_crc_check->path:%s ,fp is null\n", path);
        LOGE("\nfopen fail reason is %s\n",strerror(errno));
        return -1;
    }
    while(!feof(fp))
    {
            ...
            while(fgets(buf, sizeof(buf), fp_info))
            {
                ...
                if (sscanf(buf, "%d    %s    %u    %s", &p_number,p_name,&p_size,p_md) == 4)
                {
                    ...
                    if(strstr(p_name,path)!=NULL)
                    {
                        ...
                        if(p_cmp_name == NULL) {
                           printf("strstr path fail p_name = %s, path = %s\n", p_name, path);
                           fclose(fp_info);
                           return CHECK_FAIL;
                        }
                        if(strcmp(p_cmp_name,path)==0)
    ...
    while ((dp = readdir(d_fd)) != NULL) {
        ...
        if (dp->d_type == DT_DIR){
             ...
             for(; idy < sizeof(file_to_pass)/sizeof(char*); idy++){
               if(strstr(dp->d_name, file_to_pass[idy]) != NULL){
                        ...
                        if(p_number==number)
                            {
                                ...
                                if(p_cmp_name == NULL) {
                                  printf("Can NOT find system in p_name = %s\n",p_name);
                                  fclose(fp_info);
                                  return -1;
                                }
                                for(; idy < sizeof(file_to_pass)/sizeof(char*); idy++)
...
static void init_root_check(img_info_t *image_init)
{
    int i = 0;
    for(i=0;i<PART_MAX;i++,image_init++)
    {
//       printf("image_init->img_devname = %s \n", image_init->img_devname);
       Volume* v = volume_for_path(image_init->img_devname);
       if(v != NULL)
       {
         strcpy(image_init->img_devname,v->blk_device);
//         printf("blk_device =  %s,image_init->img_devname = %s \n", v->blk_device,image_init->img_devname);
       }
    }
}
